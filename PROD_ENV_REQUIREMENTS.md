...
| `APPSHEET_APP_ID` | Future AppSheet pharmacy gateway | Planned service module | Pending AppSheet details | AppSheet should sit behind Mission WLW so pharmacy keeps one stable Mission WLW URL. See `docs/APPSHEET_GATEWAY.md`. |
| `APPSHEET_API_KEY` | Future AppSheet API auth | Planned service module | Pending AppSheet details | Store as secret. Do not share in chat. |
| `APPSHEET_BASE_URL` | Future AppSheet API base override | Planned service module | Pending AppSheet details | Optional if using AppSheet default API base. |
