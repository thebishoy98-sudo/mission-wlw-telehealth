...
jest.mock("@/lib/db.server", () => ({ sql: jest.fn(), productDb: { getBySlug: jest.fn() } }));
...
beforeEach(() => {
  ...
  (productDb.getBySlug as jest.Mock).mockResolvedValue({ id: "tirz", isActive: true, doses: [{ id: "dose", price: 349, weeklyDoseMg: 2.5 }] });
});
...
test("rejects an invalid dose", async () => {
  expect((await POST(request({ providerApproved: true, providerName: "Provider", doseId: "invalid" }))).status).toBe(400);
  expect(sql).not.toHaveBeenCalled();
});
test("reports a concurrent change instead of claiming success", async () => {
  (sql as jest.Mock).mockResolvedValue({ rows: [] });
  expect((await POST(request({ subscriptionId: "sub", providerApproved: true, providerName: "Provider", doseId: "dose" }))).status).toBe(409);
});
test("saves a validated replacement", async () => {
  (sql as jest.Mock).mockResolvedValue({ rows: [{ id: "sub" }] });
  expect((await POST(request({ subscriptionId: "sub", providerApproved: true, providerName: "Provider", doseId: "dose" }))).status).toBe(200);
});
